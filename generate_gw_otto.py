...
# Load JSON content
with open("quran_content.json", "r", encoding="utf-8") as f:
    data = json.load(f)

# Define the Ottoman Bold Pop CSS separately to avoid f-string bracket conflicts
ottoman_styles = """
<style>
    ...
    body {
        background-color: #1A1714; /* Deep Obsidian */
        color: #F2E8CF; /* Parchment Cream */
        font-family: 'Lora', serif;
    }

    /* --- SAFFRON GLOW BOLD FONTS --- */
    strong, b {
        color: #FFC107;
        font-weight: 700;
    ...
    .surah-card {
        ...
        border-left: 4px solid #BC9353; /* Ottoman Gold */
        padding: 1.5rem;
    ...
    .tag-ottoman {
        background-color: #38A3A5; /* Iznik Turquoise */
        color: #1A1714;
        font-weight: 600;
        font-size: 0.75rem;
    ...
    if os.path.exists(md_file):
        with open(md_file, "r", encoding="utf-8") as f_md:
            post_content = markdown.markdown(
                f_md.read(),
                extensions=["extra", "codehilite", "tables", "fenced_code", "nl2br", "footnotes"]
            )
    else:
        post_content = "<p>Content file not found.</p>"

    # Create Individual Surah Page
    post_page_html = f"""
<!DOCTYPE html>
<html lang='tr'>
<head>
...
"""
    with open(html_file, "w", encoding="utf-8") as f_out:
        f_out.write(post_page_html)

    # Add card to Index string
    html_index += f"""
...
# Close Index and add Search Logic
html_index += """
        </div>
    </div>
...
print(f"✅ Success! Generated index.html and {len(data['posts'])} surah pages.")